      ...
      <div className="grid gap-px overflow-hidden rounded-sm border border-paper/8 bg-paper/8 md:grid-cols-2">
        {projects.map((p, i) => (
          ...
          >
            ...
            <span
              aria-hidden
              className="pointer-events-none absolute -right-6 -bottom-10 select-none font-display text-[9rem] leading-none text-paper/4 transition-all duration-500 group-hover:text-moss/15 group-hover:-translate-y-2"
            >
              {p.kanji}
            </span>
            <div className="relative">
              <span className="font-mono text-xs text-paper-faint">
                {String(i + 1).padStart(2, "0")} · {p.kanji} {p.gloss}
              </span>
              <h3 className="mt-4 font-display text-2xl text-paper transition-colors group-hover:text-celadon md:text-3xl">
                {p.title}
              </h3>
              ...
              <span className="mt-8 inline-block text-sm text-moss opacity-0 transition-all duration-300 group-hover:translate-x-1 group-hover:opacity-100">
                Visit →
              </span>
            </div>
          </motion.a>
